...
**Installation only needs to happen once**.  If you are using the [CSUMB Cal ICOR Server](csumb.jupyter.cal-icor.org) then the above packages are already installed and do not need to be installed again.
...
![Figure 1: Generic Goal Template](images/formula-goal.png){fig-align="center" fig-alt:"Text that provides the template needed to run the functions used in the mosiac and ggformula packages in R. It states "goal(formula, data = mydata, ...)"}
...
![Figure 2: Generic Argument Syntax](images/options.png){fig-align="center" fig-alt:"Text that provides the template needed to add arguments to the functions used in the mosiac and ggformula packages in R. It states "option = value"}
...
![Figure 3: Adding Argumetns into Functions Syntax](images/full-formula.png){fig-align="center" fig-alt:"Text that provides the template needed to run the functions used in the mosiac and ggformula packages in R with additional arguments. It states "goal(formula, data = mydata, options = value, ...)"}
...
![Figure 4: All Possible Generic Function Syntax](images/all-three-formulas.png){fig-align="center" fig-alt:"Text that provides the template needed to run the functions used in the mosiac and ggformula packages in R. It states "goal(y ~ x, data = mydata, ...)", then "goal( ~ x, data = mydata, ...)", "goal(y ~ x | z, data = mydata, ...)"}
...
study2 <- mutate(study, day = factor(day, levels = c("M", "T", "W", "R", "F", "Sa", "Su")))
```
...
![Figure 5: Generic Stored Object and Assignment Syntax*](images/object-assigned-data.png){fig-align="center" fig-alt:"Text that provides the template needed to run the functions used in the mosiac and ggformula packages in R but assign them to an object name. It states "object <- goal(formula, data = mydata, ...)"}
