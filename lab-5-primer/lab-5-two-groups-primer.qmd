...
#### Calculate the summary statistics for each sample. 
Modify the code below to calculate the summary statistics for each group. 
```{webr}
#| label: summary-stats
#| exercise: ex_2
...
Remember to
- Discuss the pattern compared to the 1:1 line that represents perfect normality  
- Describe specific details about the pattern such as if there are any deviations, where the deviations are located along the 1:1 line, and the magnitude of the deviations  
- Evaluated the severity of the deviations from the 1:1 line relative to the sample size, which should be cited.  
- Discuss the cumulative frequency rate of increase of the data points/residuals relative to the 1:1 line  




##### Based on the QQ Plot for Male Professors, which of the following are true statements (select all that apply).
...
#| echo: false
check_question(answer = c("sd"), 
               options = c("The deviations from the 1:1 line are due to the discrete nature of", "median", "IQR", "sd"), 
               type = "checkbox", 
               random_answer_order = TRUE)
```
<br>

##### Based on the QQ Plot for Female Professors, which of the following are true statements.
```{r}
#| label: qqplot-female
#| echo: false
check_question(answer = c("sd"), 
...
gf_boxplot(~(FatherAge - MotherAge), data = parents,
      xlab = "Difference between Fahter's and Mother's Age of Newborn Infants in 2023") 
```
