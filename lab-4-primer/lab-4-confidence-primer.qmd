...
First, be sure you have installed `ggformula` and `mosaic`. Remember, you only need to install each package once on your machine. 

Then, be sure to load the packages `ggformula` and `mosaic`, using the `library()` function. Remember, you need to do this with each new Quarto/RMarkdown document or R Session. Add the package names in each of the blanks below to load in the indicated packages. 
...
```{webr}
#| exercise: ex_2
bee <- ________("HoneyBeeColonies_since2000.csv")
```
...
Here is are a few rows of the data: 
...
checkdown::check_question(options = c("size of honeybee colony", 
                          ...
                          "number of honeybee colonies in the US", 
                          ...
                          "numeric discrete", 
                          "numeric continuous"), 
               answer = c("number of honeybee colonies in the US", 
                          "numeric discrete"), 
               right = "Great job!", 
...
##### Question 7: Is there a y-axis in a single numeric variable boxplot? 
...
::: {callout note}
<!--- these use the graphics only so I put them here. Should all questions be at the end under 'check your understanding? --->


#### Question 8. What is the shape and modality of these data? 
...
#### Question 9. Based on the shape, the mean should be _______ the median.  
...
Use the code chunk below to calculate some summary statistics. 

```{webr}
#| exercise: ex_7
#| caption: Calculate summary statisics

```


```{r, eval=FALSE, echo=FALSE}
df_stats(~Colonies, data = bee)
```
...
#### Question 10. How many cases have between 2301 and 2504 thousand colonies? 
...
#### Question 11. What is the name for the distance between 2504 and 2672.75 thousand colonies on the boxplot? 

```{r}
#| label: eda-q11
#| echo: false
checkdown::check_question(answer = c("inter-quartile range"), 
                          placeholder = "write answers in all lower case",
                          right = "Great job!", 
...
#### Question 12. There are more cases that have values between 2301 and 2504 thousand colonies than cases that have values between 2672.75 and 2812 thousand colonies. 
...
#### Question 13. The cases that have values between 2301 and 2504 thousand colonies are more dispersed then the cases that have values between 2672.75 and 2812 thousand colonies. 
...
#### Question 14. We could have determined the modality of the distribution without making a histogram (i.e. with just statistics or the boxplot). 
...
```{r stat-output, echo=FALSE}
df_stats(~Colonies, data = bee)
```

#### Question 15. From this output identify each value appropriatly: 
...
checkdown::check_question(answer = 2591.3, 
               title = paste0("What is the value of ", expression(bar(x)), "?"),
               button_label = "Check Your Answer")
```
...
```{r, results = FALSE}
xbar<-mean(~Colonies, data = bee)
s<-sd(~Colonies, data = bee)
n<-length(bee$Colonies)
