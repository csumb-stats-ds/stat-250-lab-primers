...
### Modify the code below to estimate the population regression line using the data and find the slope and intercept.  Hint: use the functions `lm()` and `coef()`
...
If you wanted to save yourself some hand calculations, you could use the `predict()` function in R.  

```{webr}
predict(mm_age, newdata = c(age = 60))
```
:::