...
- `group_variable ~ numberic_variable` 
...
There are a lot of functions we learned for summary statistics, include:  
...
Remember to include `na.rm = TRUE` if you have missing data to avoid getin `NA` as your answer. 


Caclulate the mean for body mass for penguins.  
...
You can also calculate the mean, or other statistics, split across multiple groups, such as species. Try replacing `mean()` with other functions. Remember to add `na.rm = TRUE` if you get `NA` as an output.  
